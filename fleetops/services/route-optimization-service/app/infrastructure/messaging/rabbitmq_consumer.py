...
MensajeHandler = Callable[[str, dict], Awaitable[None]]


class RabbitMQConsumer:
    """Se suscribe a un exchange topic de RabbitMQ (con soporte de wildcards en
    la routing key, p.ej. 'vehiculo.*') y despacha cada mensaje deserializado,
    junto con su routing key, al handler dado. Corre como tarea de fondo dentro
    del lifespan de la aplicacion."""
            ...
            async for mensaje in queue_iter:
                async with mensaje.process(ignore_processed=True):
                    try:
                        ...
                        await handler(mensaje.routing_key, payload)
                    except Exception:
                        logger.error(
