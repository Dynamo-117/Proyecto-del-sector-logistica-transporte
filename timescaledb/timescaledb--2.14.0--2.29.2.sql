...
) RETURNS regprocedure AS '$libdir/timescaledb-2.29.2', 'ts_continuous_agg_get_bucket_function' LANGUAGE C STRICT VOLATILE;
...
CREATE FUNCTION _timescaledb_functions.compressed_data_info(_timescaledb_internal.compressed_data)
    ...
    AS '$libdir/timescaledb-2.29.2', 'ts_update_placeholder'
...
AS '$libdir/timescaledb-2.29.2', 'ts_update_placeholder' LANGUAGE C STRICT;
...
) RETURNS REGCLASS AS '$libdir/timescaledb-2.29.2', 'ts_update_placeholder' LANGUAGE C VOLATILE;
...
CREATE PROCEDURE @extschema@.add_columnstore_policy(
    ...
    hypercore_use_access_method BOOL = NULL
) LANGUAGE C AS '$libdir/timescaledb-2.29.2', 'ts_update_placeholder';
...
AS '$libdir/timescaledb-2.29.2', 'ts_update_placeholder' LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;
...
CREATE PROCEDURE @extschema@.convert_to_columnstore(
    ...
    if_not_columnstore BOOLEAN = true,
    recompress BOOLEAN = false
) AS '$libdir/timescaledb-2.29.2', 'ts_update_placeholder' LANGUAGE C;
...
CREATE FUNCTION timescaledb_experimental.add_policies(
    ...
    compress_after "any" = NULL,
    drop_after "any" = NULL
) RETURNS BOOL AS '$libdir/timescaledb-2.29.2', 'ts_update_placeholder' LANGUAGE C VOLATILE;
...
CREATE OR REPLACE FUNCTION @extschema@.delete_job(job_id INTEGER) RETURNS VOID AS '$libdir/timescaledb-2.29.2', 'ts_job_delete' LANGUAGE C VOLATILE STRICT;
CREATE OR REPLACE PROCEDURE @extschema@.run_job(job_id INTEGER) AS '$libdir/timescaledb-2.29.2', 'ts_job_run' LANGUAGE C;
...
CREATE OR REPLACE FUNCTION @extschema@.remove_compaction_policy(hypertable REGCLASS, if_exists BOOL = false) RETURNS VOID
